module Watir
  class IE
    ...
    def load_default_waiters
      waiter = Proc.new { check_for_closed_ie_window {sleep @interval while @ie.busy} }
      add_waiter(:ie_busy, waiter, 10)

      waiter = Proc.new{check_for_closed_ie_window {sleep @interval until READYSTATES.has_value?(@ie.readyState)}}
      ...
      waiter = Proc.new{check_for_closed_ie_window {sleep @interval until @ie.document && @ie.document.body}}
