...
class TC_Frames < Test::Unit::TestCase
  ...
  def test_frame_using_index
    assert_raises(UnknownObjectException) { browser.frame(:index, 7 + Watir::IE.base_index + Watir::IE.base_index).button(:id, "b2").enabled?  }
    assert_raises(UnknownObjectException) { browser.frame(:index, 1 + Watir::IE.base_index + Watir::IE.base_index).button(:id, "b2").enabled?  }
    assert(browser.frame(:index, 0 + Watir::IE.base_index ).button(:id, "b2").enabled?)
    assert_false(browser.frame(:index, 0 + Watir::IE.base_index ).button(:caption, "Disabled Button").enabled?)
    assert_equal('blankpage.html', browser.frame(:index, 1 + Watir::IE.base_index ).src)
  end
...
class TC_NestedFrames < Test::Unit::TestCase
  ...
  def test_frame
    ...
    browser.frame(:name => "nestedFrame").frame(:name => "senderFrame").text_field(:index, 0 + Watir::IE.base_index ).set("Hello")
...
class TC_IFrames < Test::Unit::TestCase
  ...
  def test_Iframe
    ...
    browser.frame(:name => "senderFrame").button(:index, 0 + Watir::IE.base_index ).click
...
class TC_frames_method_for_container < Test::Unit::TestCase
  ...
  def test_frames_collection
    ...
    assert_equal('first_frame', frames[0 + Watir::IE.base_index].id)
    assert_equal('pass.html', frames[2 + Watir::IE.base_index].src)
    assert_nothing_raised {frames[0 + Watir::IE.base_index].html}
  end
end
...
class TC_iframe_access < Test::Unit::TestCase
  ...
  def test_frame_without_access_should_still_show_properties
    ...
    assert_raises(FrameAccessDeniedException) {frame.button(:index, 0 + Watir::IE.base_index).click}
  end
